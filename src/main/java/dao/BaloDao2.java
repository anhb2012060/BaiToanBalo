 ...
 * @author OS
 */
public class BaloDao2 {
    public static boolean insertBalo (Balo balo){
        String sql = "insert into balo2(ma,trongLuongBalo) values (?,null)";
        try {
    ...
    public static boolean updateBalo(double trongLuongBalo, String maBalo){
        String sql = "update balo2 set trongLuongBalo = ? where ma = ?";
        try {
            Connection con = DatabaseHelper.openConnection();
            PreparedStatement pstmt = con.prepareStatement(sql);
    ...
    public static boolean updateSttBalo(String ma, int stt){
        String sql = "update balo2 set soThuTu = ? where ma = ?";
        try {
            Connection con = DatabaseHelper.openConnection();
            PreparedStatement pstmt = con.prepareStatement(sql);
            pstmt.setInt(1, stt);
    ...
    public static Balo findById(String maBalo){
        String sql = "select trongLuongBalo,soThuTu from balo2 where ma = ?";
        try {
            Connection con = DatabaseHelper.openConnection();
            PreparedStatement pstmt = con.prepareStatement(sql);
    ...
    public static List<String> findAllIdBalo(){
        String sql = "select ma from balo2";
        try {
            Connection con = DatabaseHelper.openConnection();
            PreparedStatement pstmt = con.prepareStatement(sql);
            ResultSet rs = pstmt.executeQuery();
